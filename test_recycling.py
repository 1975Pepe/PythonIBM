...
class TestRecycling(unittest.TestCase):
    ...
    def test_get_crate_quantities(self):
        ...
        with patch('builtins.input', side_effect=example_data) as mock_input:
            self.assertEqual(
                recycling.get_crate_quantities(6), example_data)
    ...
    def test_int_input(self):
        ...
        with patch('builtins.input', side_effect=['-2', '-1000', 'abc', '123abc', '3']) as mock_input:
            self.assertEqual(
                recycling.positive_int_input('example question'), 3)
        ...
        with patch('builtins.input', side_effect=['0', '13', '1', '100000000']) as mock_input:
            self.assertEqual(
                recycling.positive_int_input('example question'), 0)
            self.assertEqual(recycling.positive_int_input(
                'example question'), 13)
            self.assertEqual(
                recycling.positive_int_input('example question'), 1)
            self.assertEqual(recycling.positive_int_input(
                'example question'), 100000000)
